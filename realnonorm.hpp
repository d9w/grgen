#ifndef REALNONORM_HPP
#define REALNONORM_HPP
#include <iostream>
#include <array>
#include <vector>
#include <utility>
  ...
  RealNoNorm() {}
	...
	template <typename GRN> void updateSignatures(GRN& grn) {
		...
		for (size_t i = 0; i < grn.actualProteins.size(); ++i) {
			for (size_t j = 0; j < grn.actualProteins.size(); ++j) {
				grn.signatures[i][j] = {
				    {max(0.0, exp(grn.params[0] * grn.signatures[i][j][0] - 1.0)),
				     max(0.0, exp(grn.params[0] * grn.signatures[i][j][1] - 1.0))}};
			}
		}
	}
	...
	template <typename GRN> void step(GRN& grn, unsigned int nbSteps) {
		for (auto s = 0u; s < nbSteps; ++s) {
			...
			for (size_t j = grn.getFirstRegulIndex(); j < grn.getNbProteins(); ++j) {
				...
				nextProteins.push_back(min(1.0, max(0.0, grn.actualProteins[j].c +
